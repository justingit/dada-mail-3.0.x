...
#	 die scalar localtime($form_vals{mailing_date});
	...
	my $tmp_record = {};
	if($q->param('key')){
		$tmp_record = $mss->get_record($q->param('key'));
	}
...
sub file_test { 
	...
	$r .= $q->p({-class => 'error'}, 'Can\'t read file!') unless -r $fn; 
	$r .= $q->p({-class => 'error'}, 'File doesn\'t exist!') unless -e $fn; 	
...
Example: To run the schedules for "mylistshortname": 
	...
	./schedule_mailings.pl --run --list mylistshortname
