...
sub send_email { 
   ...
   foreach my $field(@{$lh->subscriber_fields({-dotted => 1})}){ 
	...
	if(! $process){
		...
		my ($num_list_mailouts, $num_total_mailouts, $mailout_will_be_queued)  = $self->mass_mailout_info($list);
  ...
  #  warn '$attachment ' . $attachment; 
    ...
    #}else{ 
        ...
        if($DADA::Config::ATTACHMENT_TEMPFILE == 1){ 
            ...
            my $attachment_file = $self->file_upload({-cgi_obj => $q, -name => $name}); 
...
sub mass_mailout_info { 
	...
	if($@){ 
		...
		if($DADA::Config::MAILOUT_AT_ONCE_LIMIT ne undef){ 
			if(($num_total_mailouts - $paused_mailouts) >= $DADA::Config::MAILOUT_AT_ONCE_LIMIT){ 
				$mailout_will_be_queued = 1;
			}
		}
	}
	return($num_list_mailouts, $num_total_mailouts, $mailout_will_be_queued); 
