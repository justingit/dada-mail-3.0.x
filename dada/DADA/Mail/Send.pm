	   ...
	   $local_li->{send_via_smtp}        == 1       
         ...
         $self->_pop_before_smtp;
           ...
           if( $local_li->{set_to_header_to_list_address} == 1 && 
            ...
            close(MAIL) 
                or carp "$DADA::Config::PROGRAM_NAME $DADA::Config::VER Warning: didn't close pipe to '$live_mailing_settings' while attempting to send a message to: '" . $fields{To} ." because:' $!";  
	...
	FORK: {
		if ($pid = fork) {
			...
			while(defined($mail_info = <MAILLIST>)){ 	
				...
				$self->send(%nfields, from_mass_send => 1); # The from_mass_send is a hack. 
               ...
               if($mailing_count != $new_count){ 
                    carp("\$mailing_count ($mailing_count) is not the same as \$new_count ($new_count) - problems are likely to happen..."); 
               }
