...
http://dadamailproject.com/support/requirements.html

=head2 But I have a Windows Server, The system requirements B<clearly> state that it's not supported - is this true? 

Short answer: B<yes>

Long answer: 

http://dadamailproject.com/support/documentation/README-windows.pod.html
...
The largest hurtle is going to be emulation of the C<fork()> Perl subroutine. 
...
Yes. You can rename the, B<mail.cgi> file to any other valid filename, like newsletter.cgi or craaaaazyname.cgi. You'll most likely have to keep the file ending either, B<.cgi> or B<.pl>

The only place that this variable is hardcoded is in the, B<PROGRAM_URL> variable - so make sure to change this variable as well to reflect the file name changes.
...
=item * Killing the sending process

If you have shell access, issue the B<kill> command with the process id (pid) of the sending process. This id should be writtern down in Dada Mail's usage log - if you haven't set this log explicitly, it should be saved as, I<dada_usage.txt> in whatever directory you set the B<$LOGS> variable to - if you haven't set that variable explicitly, that variable will be set to whatever B<$FILES> is set to (*whew*!)
...
The message pid for this message is, B<76354>. You'll then want to type in the B<kill> command: 

 kill 76354

 The exact format of issuing the command my be different for your server, but that's the jist of it. 

=item * Deleting the temporary subscriber list
...
This file is saved in whichever directory you set the, B<$TMP> variable to. If you haven't set the variable, it'll be saved in the directory you set the, B<$FILES> variable to. It looks sort of like this: 

 mylist.list.20051120131106.78619561_at_example.com

The format is, your list shortname, the type of list you're sending (usually, 'list'), a whole string of numbers that correspond to the B<Message-id> header of the message you're sending out. 
...
New subscribers should be prepared in CSV format, with the email address being first and any other subscriber fields coming afterwards. The actual order will be listed on this screen. 

If you do not have any subscriber fields, the format needed to give Dada Mail is just, one email address and a newline. 
...
You can change the unsubscription link from what's above, to: 
 ...
 [list_unsubscribe_link]/[subscriber.email_name]/[subscriber.email_domain]/
...
http://dadamailproject.com/support/documentation/UPGRADE_README.pod.html
...
=item * The, "Send a List Message" Basic/Advanced screen

In the list control panel, go to, B<Send a List Message>. In the basic screen, there is a pulldown menu labeled, B<Format:> Select, B<HTML> and write your message in HTML.
...
Once that's accomplished, you'll want to set Dada Mail's Config.pm variable, C<$FCKEDITOR_URL> to point to the location of the fckeditor directory. Once that's set and you can see the FCKeditor in the, "Send a List Message" screen, when the, "HTML Version" textarea is, you're ready to move on. 
...
=item * Click, "Show/Hide Advanced Options" and select,
...
For example, the, "Send a List Message" screen, has a flavor parameter query string of, I<send_email>. The template that creates this screen is called, I<send_email_screen.tmpl> Easy enough. 
...
Either, don't edit the individual copy of the list template in the admin control panel under, B<Manage Appearance - Edit Template>, check the option, " Use the default list template."  in that same screen, or, to be even more authorative, add this to your %LIST_SETUP_OVERRIDES Config.pm hash: 
...
Email messages are controlled per list. In the list control, go to: B<Manage List - Sending Options - Advanced> and select the charset you'd like to use under, B<Default Character Set of Mailings>
...
There's a script called, B<dada_settings_dbtosql.pl> which is located in: 
...
There's a script called, B<dada_archive_dbtosql.pl> which is located in: 
...
There's a script called, B<dada_subscribers_plaintext2sql.pl> which is located in: 
...
Edit to your liking - one special tag that you're going to want to make sure you still include is the: B<[message_body]> tag. This tag will be replaced by your actually message body - meaning, what you wrote, when your mailing list message is sent. 

If your mailing list messages are coming to you as blank, make sure you did not remove the, B<[message_body]> tag!
...
The cache is, B<flushed>, or deleted every time each list changes any list setting via the list control panel, add/edit/removes an archived message in any list and when any change is made in the, "Edit Template" list control panel screen.
...
=head2 How do you edit an archived message? (2.10.7 and above) 
...
=head2 How do you edit an archived message? (2.10.6 and below)
...
=head2 How do you edit an archive message with anything that's below 2.10.2?

You can't. 

=head2 I remember that you used to be able to edit an archived message - what happened?
...
I<A web-based MIME editor, written in Perl which can take a valid, but incredibly varied format.>
...
=head2 OK, I get it, there is no way to edit archived messages (in versions below 2.10.2) - is there a workaround?
...
=item 2 Log into Dada Mail's List control panel and go to, "Send a List Message". (Re)write the message. 

=item 3 Find the link at the bottom of the page titled, B< Show/Hide Archive Options ->. Click it. 
...
=item * The, "Edit the SQL Table directly", way
...
=item * The, "Edit the archive backups and reload" way
...
=item * The, "Use an older version of Dada Mail to edit archives", way
...
=head2  Does Dada Mail support the saving of more than the email address? 
...
=head2 How is Dada Mail developed? 
...
As far as I'm concerned, I'm an absolute gear head when it comes to layout, design and usability. Not B<always> shows up in Dada Mail's design, but I try. Got that BFA and all...
...
If you'd like to start hacking away at the program, start by submitting patches to be applied to B<the latest version checked out from the CVS server>. 
...
=head2 Is there a CVS server I can check in/out of? 

No. We've moved to Git. That information can be accessed here: 

L<http://dadamailproject.com/download/git.html>
