...
You may enable the mass-subscription of subscribers in the list control panel as well, by logging into the list control panel with the Dada Mail Root Password and going to: I<Manage List Control Panel - Options> and checking the option labeled, I<Enable, "Subscribe Checked Subscribers" button in the, Manage Subscribers -> Add -> Verify> screen. 
...
There's really good reasons why you should not mass-subscribe people for any reason, except for importing an already existing and I<verified> list. Other than that, it should never, ever be used and we'd like to hit that point home, as people, who shall remain nameless, can't possibly get a clue on why it could be a bad idea. 
